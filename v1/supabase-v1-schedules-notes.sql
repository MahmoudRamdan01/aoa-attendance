-- =====================================================================
--  Air Ocean Line — v1: per-employee check-in/out windows + attendance notes
--  Run after supabase-v1-migration.sql. Safe to rerun (idempotent).
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1) Schema: per-employee window overrides (NULL = use global settings)
--            + employee/HR notes on the daily attendance record.
-- ---------------------------------------------------------------------
alter table employees  add column if not exists checkin_from  time;
alter table employees  add column if not exists checkin_to    time;
alter table employees  add column if not exists checkout_from time;
alter table employees  add column if not exists checkout_to   time;

alter table attendance add column if not exists employee_note text;  -- written by the employee at check-in
alter table attendance add column if not exists hr_note       text;  -- written by HR next to the name

-- ---------------------------------------------------------------------
-- 2) Global windows: check-in 08:00–11:00, check-out 16:30–20:00.
-- ---------------------------------------------------------------------
insert into settings(key,value,note) values
  ('checkin_from','"08:00"','بداية نافذة الحضور'),
  ('checkin_to','"11:00"','نهاية نافذة الحضور'),
...
update settings set value='"16:30"' where key='checkout_from';
update settings set value='"20:00"' where key='checkout_to';

-- ---------------------------------------------------------------------
-- 3) Per-employee override: عبدالرحمن checks in 14:00–16:00 (afternoon shift).
--    Checkout stays on the team window (16:30–20:00).
-- ---------------------------------------------------------------------
update employees set checkin_from='14:00', checkin_to='16:00' where name = 'عبدالرحمن';

-- ---------------------------------------------------------------------
-- 4) employee_attendance_action_v1 — now enforces the check-in/out window
--    (per-employee override or global) and stores the employee's note.
--    Signature gains p_note; drop the old 6-arg version first.
-- ---------------------------------------------------------------------
drop function if exists employee_attendance_action_v1(text,numeric,numeric,int,text,text);
...
begin
  ...
  if p_kind = 'in' then
    ...
    -- Effective check-in window: employee override, else global.
    ...
    -- Effective check-out window: employee override, else global.
...
grant execute on function employee_attendance_action_v1(text,numeric,numeric,int,text,text,text) to authenticated;

-- ---------------------------------------------------------------------
-- 5) HR note next to the employee (visible to Owner, persisted).
-- ---------------------------------------------------------------------
create or replace function set_attendance_note_v1(p_employee_id bigint, p_date date, p_note text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
  v_note text := nullif(trim(coalesce(p_note,'')),'');
begin
  ...
  insert into attendance(employee_id, work_date, status, source, approved, hr_note)
  values (p_employee_id, p_date, 'pending', 'hr', false, v_note)
  on conflict (employee_id, work_date) do update set hr_note = v_note;

  insert into audit_log(actor,action,entity,entity_id,details)
  values (auth.uid(),'set_hr_note','attendance',p_employee_id::text,jsonb_build_object('date',p_date));
  return jsonb_build_object('ok',true);
end $$;
grant execute on function set_attendance_note_v1(bigint,date,text) to authenticated;

-- ---------------------------------------------------------------------
-- 6) Auto-absence must respect per-employee check-in windows.
--    عبدالرحمن checks in 14:00–16:00; the daily cron runs at 11:00–12:00
--    Cairo, before his window opens. Only mark someone absent once their
--    effective check-in window has actually closed (else we'd flag the
--    afternoon shift as absent every morning).
-- ---------------------------------------------------------------------
create or replace function mark_absentees_v1(p_date date default null)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
  v_now  timestamp := now() at time zone 'Africa/Cairo';
  v_date date := coalesce(p_date, v_now::date);
  v_time time := v_now::time;
  v_g_to time := (select (value#>>'{}')::time from settings where key='checkin_to');
  v_count int := 0;
begin
  ...
  from employees e
  where e.active
    -- لو بنعلّم غياب لليوم نفسه: نتأكد إن نافذة حضوره (الخاصة أو العامة) خلصت.
    -- لو بنعلّم يوم فات (p_date قديم) نعلّم عادي.
    and (
      v_date < v_now::date
      or coalesce(e.checkin_to, v_g_to) is null
      or v_time > coalesce(e.checkin_to, v_g_to)
    )
    and not exists (
...
-- ---------------------------------------------------------------------
-- 7) Least-privilege: block anon from the newly (re)created functions.
--    On Supabase, fresh functions get EXECUTE granted to PUBLIC (plus
--    anon/authenticated/service_role), and anon inherits the PUBLIC grant —
--    so revoking from anon alone is NOT enough; PUBLIC must be revoked too.
--    authenticated keeps the explicit grant above (signed-in employee/HR).
-- ---------------------------------------------------------------------
revoke execute on function employee_attendance_action_v1(text,numeric,numeric,int,text,text,text) from public, anon;
revoke execute on function set_attendance_note_v1(bigint,date,text) from public, anon;
revoke execute on function mark_absentees_v1(date) from public, anon;
